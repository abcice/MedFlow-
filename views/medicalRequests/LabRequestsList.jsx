...
function LabRequestsList({ requests, token }) {
    return (
        <Layout token={token}>
            <div className="container" style={{ maxWidth: '1000px', margin: '0 auto' }}>
                <h1 style={{ marginBottom: '20px' }}>🧪 Lab Requests</h1>

                {/* Search Form */}
                <form 
                    method="GET" 
                    action={`/medicalRequests/lab`} 
                    ...
                    <tbody>
                        {requests.map(req => (
                            <tr key={req._id}>
                                ...
                                <td>
                                    <a 
                                        href={`/medicalRequests/lab/${req._id}?token=${token}`} 
