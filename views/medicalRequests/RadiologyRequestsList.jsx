...
function RadiologyRequestsList({ requests, token, userRole }) {
    ...
    return (
        <Layout token={token}>
            ...
            <form method="GET" action="/medicalRequests/radiology" style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
                        ...
                        {requests.map(req => (
                            <tr key={req._id}>
                                ...
                                <td style={{ display: 'flex', gap: '5px' }}>
                                    <a href={`/medicalRequests/radiology/${req._id}?token=${token}`} className="btn btn-sm btn-primary">View</a>
                                    {userRole === 'Doctor' && (
                                        <form method="POST" action={`/medicalRequests/radiology/${req._id}?_method=DELETE&token=${token}`} onSubmit={(e) => { if (!confirm('Are you sure you want to delete this request?')) e.preventDefault(); }}>
