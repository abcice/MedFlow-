...
function RadiologyRequestDetail({ token, request, userRole }) {
    return (
        <Layout token={token}>
            ...
            {userRole === 'Doctor' && (
                <form method="POST" action={`/medicalRequests/radiology/${request._id}?_method=DELETE&token=${token}`} 
                      onSubmit={(e) => { if (!confirm('Delete this radiology request?')) e.preventDefault(); }}>
